 ...
 */
function hook_content_moderation_access_alter(&$access, $op, $node) {
  global $user;
 ...
 */
function hook_content_moderation_states_next_alter(&$states, $current_state, $context) {
 ...
 */
function hook_content_moderation_transition($node, $previous_state, $new_state) {
  // Your code here.
}
 ...
 */
function hook_content_moderation_state_save($state, $status) {
 ...
 *   on if this INSERT'ing a new transation or UPDATE'ing an existing one.
 */
function hook_content_moderation_transition_save($transition, $status) {
  if ($status == MergeQuery::STATUS_INSERT) {
    // Add data to a custom table for each new transition.
    db_insert('mytable')
      ->fields(array(
 ...
 */
function hook_content_moderation_state_delete($state) {
 ...
 */
function hook_content_moderation_transition_delete($transition) {
  // Remove data from a custom table which refers to the old state.
  db_delete('mytable')
